namespace NhaTro.Services
{
    public class AssistantActionRegistry
    {
        ...
        private static IEnumerable<AssistantActionDefinition> BuildActions()
        {
            ...
            yield return new() { Intent = ContractsCreate, Description = "create a contract for a room; select a matching tenant or create the tenant when no match exists", RequiredFields = new[] { "roomCode", "tenantName", "startDate", "actualRoomPrice", "occupantCount" }, RequiresConfirmation = true, Examples = new[] { "tạo hợp đồng phòng A1 cho Nguyễn Văn A từ 1/7 giá 3000000" } };
            ...
            yield return new() { Intent = ContractsUpdate, Description = "update active contract details; identify it by contractId or roomCode", OptionalFields = new[] { "roomCode", "contractId", "startDate", "expectedEndDate", "depositAmount", "depositPaidAmount", "occupantCount", "actualRoomPrice" }, RequiresConfirmation = true, Examples = new[] { "sua tien coc hop dong phong A1 thanh 5 trieu", "doi so nguoi o hop dong phong B2 la 3" } };
